...
program main
  ...
  real(8) :: vinil,Tinil, rhoinil,xinil,vinir,Tinir, rhoinir,xinir,xinim, mach
  real(8) :: vshock_ad,Tshock_ad,vshock_iso,Tshock_iso,xshock_ad, xshock_iso, rho_ad, T_ad, rho_iso, T_iso, xr
  integer :: icore(1:nb+mergin), numr,numl
  ...
  mach = vinil/dsqrt(gamma*boltz*Tinil/rmasp/mu)
  
  numl = int(dble(nb_1dx)*(xinim-xinil)/(xinir-xinil))
  numr = nb_1dx - numl
  i=0
...
!              kappa(i) = 4d-40
              ...
              xb(1,i) = dble(ix-mergin_1dx) / dble(nb_1dx) * (xinir-xinil) + xinil
              xb(2,i) = 0d0
              xb(3,i) = 0d0
              ...
              if(xb(1,i) > xinim) then
                 vbx(1,i) = vinir
                 db(i)=rhoinir
                 mass(i) = db(i)*(xinir-xinim)/dble(numr)/6.8602122318752234d-2*rhoinir
                 th_energy(i) = Tinir * cv(i) ! Thermal energy per unit mass
              else
                 vbx(1,i) = vinil
                 db(i)=rhoinil
                 mass(i) = db(i)*(xinim-xinil)/dble(numl)/1.0002540263171265d-2*rhoinil
                 th_energy(i) = Tinil * cv(i) ! Thermal energy per unit mass
              endif
...
! 1D resize Whitehouse & Bate 2004 ---------
  ...
  do i=1,nbtot
     if(iboundary(i)==1)then
        if( i < mergin_1dx+1) then
           sd(i) = sd(mergin_1dx+1)
        else
           sd(i) = sd(nbtot_1dx-mergin_1dx-1)
        endif
     endif
  enddo
