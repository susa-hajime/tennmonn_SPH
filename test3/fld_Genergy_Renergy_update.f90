...
!     write(*,*) '2 icount=',icount,i


     ! Does i-th particle have neighbors? 
     if( neicnt(i)/= 0 ) then 
        do jj = 1 , neicnt(i)
!           write(*,*) '2.1 icount=',icount,jj,neicnt(i)
...
!                 if(vrtot > 1d-1 * csav*dabs(rij) ) rmuij=0.d0
...
!                 dwz=dwij*dz
                 ...
                 if( dabs( lambda_fld(i)/(kappa(i)*dbb) + lambda_fld(nlp)/(kappa(nlp)*dbbb) ) < 1d-50) then 
                    bbb = 4d0/( (kappa(i)*dbb) / lambda_fld(i) + (kappa(nlp)*dbbb)/lambda_fld(nlp) )
                 else
...
!        call checknan2(uth2,'nan hogege','go',flg)
!        if(flg == 1) then
!           write(*,*) i,fitt,uth1**4,a1*uth1,a0,4d0*uth1**3,a1
!           stop
!        endif
...
113  continue
!     th_energy(i) = th_energy_old(i) + dtb(i)*dt
     
!     th_energy(i)=-a0/a1
!     th_energy(i)=(th_energy_old(i) + Pn_fld*dt)/(1.d0-Pd_fld*dt)
!     th_energy(i)=(th_energy_old(i) + Pn_fld*dt) +Pd_fld*dt*th_energy_old(i)
