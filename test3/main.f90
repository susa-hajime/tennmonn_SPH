...
program main
  ...
  ! Initial conditions ----------------------------------------------
  Tini = 1500d0
  vini = dsqrt(gamma*boltz*Tini/(mu*rmasp))
  rhoini = 1d-10
     ...
     do iy = 1,nbtot_1dy
        do iz = 1,nbtot_1dz
           ...
           if(iy == nbtot_1dy/2+1 .and. iz == nbtot_1dz/2+1) then
              ...
              xb(1,i) = ( dble(ix)-dble(nbtot_1dx/2) -0.5 ) / dble(nb_1dx) * 2d15
              xb(2,i) = ( dble(iy)-dble(nbtot_1dy/2) -0.5 ) / dble(nb_1dy) * 2d15
              xb(3,i) = ( dble(iz)-dble(nbtot_1dz/2) -0.5 ) / dble(nb_1dz) * 2d15
              sd(i) = 1.1d0 / dble(nb_1dx) * 2d15
!              sd(i) = 2.5d0 / dble(nb_1dx) * 2d15
              ...
              if(ix >= nbtot_1dx-mergin_1dx/2 .or. ix <= mergin_1dx/2 .or.&
                   &iy >= nbtot_1dy-mergin_1dy/2 .or. iy <= mergin_1dy/2 .or.&
                   &iz >= nbtot_1dz-mergin_1dz/2 .or. iz <= mergin_1dz/2 ) then
                 iboundary(i)=1
              else
                 iboundary(i)=0
              endif
              ...
              db(i)=rhoini
              mass(i) = db(i)*2d15/dble(nb_1dx)/1.000050805263425d0
...
!              kappa(i) = 4d-10
              ...
              th_energy(i) = Tini*cv(i) ! Thermal energy per unit mass
...
! 1D resize Whitehouse & Bate 2004 ---------
 ...
 !-------------------------------------------  
  ...
  open(51,file="out_ad.dat",status="unknown")
  open(52,file="out_iso.dat",status="unknown")
...
777 continue
  ...
  do i = 0,1000
     ...
     if(dabs(xx) > xshock_ad) then
           write(51,3000) xx, rhoini,Tini
        else
           write(51,3000) xx, rho_ad,T_ad
        endif
        
        if(dabs(xx) > xshock_iso) then
           write(52,3000) xx, rhoini,Tini
        else
           write(52,3000) xx, rho_iso,T_iso
        endif
