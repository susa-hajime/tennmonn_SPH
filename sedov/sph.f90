...
!  ! $OMP PARALLEL private(i,jj,j,r)
!  ! $OMP DO schedule(static) 
  do i = 1 , nb+mergin
     
     do jj = 1, neicnt(i)
        j = neilist(jj,i)
!        r = dsqrt((xb(1,i)-xb(1,j))**2 + (xb(2,i)-xb(2,j))**2 + (xb(3,i)-xb(3,j))**2) /sd(j)
!        db(i) = db(i) + mass(j)*w( r ) / sd( j )**3 ! scatter 
...
!        r = dsqrt((xb(1,i)-xb(1,j))**2 + (xb(2,i)-xb(2,j))**2 + (xb(3,i)-xb(3,j))**2) /(sd(i)+sd(j))*2d0
!        db(i) = db(i) + mass(j)*w( r ) / (sd( j )+sd( i ))**3*8d0 !gatnher&scat
...
subroutine sph
  ...
  use define_tree, only: neilist,neicnt
  implicit none
  real(8) :: ptot, ptot2, pij, dwx,dwy,dwz,dwx2,dwy2,dwz2, cs1,cs2,dwij, dwij2, vn2inv, dwi, dwj
...
!    !$OMP PARALLEL private(ptot2, pij, dwi,dwj,dwx,dwy,dwz,dwx2,dwy2,dwz2, cs1,cs2,dwij, dwij2, vn2inv,vrtot,rmuij, dvx,dvy,dvz,dx,dy,dz, rij, rijinv, rij2, factor,sd2_i, sd2_nlp, sdav, i, jj, nlp), shared(rmaxmu)
        ...
        do jj = 1 , neicnt(i)
           ...
           if( i .ne. nlp ) then
              ...
              if(rij2 <= 4d0*sd2_nlp) then
                 ...
                 cs1=dsqrt(vn2*tb(i)*gamma)
                 cs2=dsqrt(vn2*tb(nlp)*gamma)
                 ...
                 pij = ( -aqalp * rmuij * ( cs1 + cs2 ) * 5.d-1 + aqbet * rmuij**2 )&
                      &/(db(i)+db(nlp))*2.d0*vn2inv
                 
                 ! artificial viscosity (monaghan 1997)
!                 pij = -aqalp*vsig*rmuijsig /(db(i)+db(nlp))*vn2inv
...
!                 dwij=factor*dw(rij/sdav)/sdav**4 !CAUTION ->dimension 
                 ...
                 ! pressure force ( including artificial viscosity ) ******
                 
                 ! ptot = tb(i)/db(i) + tb(nlp)/db(nlp) + pij
                 ptot2 = tb(i)/db(i) * f_h(i) * dwi + tb(nlp)/db(nlp) *f_h(nlp) * dwj
                       
                 ! ! force
                 ! dpx(1,i)=dpx(1,i)-ptot*dwx
                 ! dpx(2,i)=dpx(2,i)-ptot*dwy
                 ! dpx(3,i)=dpx(3,i)-ptot*dwz
                 ! conservative
                 ...
                 dtb(i) = dtb(i) + (gamma-1d0)*( tb(i) / db(i) + pij * 5.d-1 )* (dvx * dwx2 + dvy*dwy2 +dvz*dwz2)
!                 dtb(i) = dtb(i) + (gamma-1d0)*( tb(i) / db(i) + pij * 5.d-1 )* (dvx * dwx + dvy*dwy +dvz*dwz)

                 dAs(i) = dAs(i) + (gamma-1d0)/db(i)**(gamma-1d0) *(  pij * 5.d-1 )* (dvx*dwx + dvy*dwy +dvz*dwz)

!                 if(i <= nb/2 .and. i > nb/4) then
...
!                    write(*,*) pij,rmuij
!                    write(*,*) cs1,cs2
!                    write(*,*) dpx(1,i),dpx(2,i),dpx(3,i)
!                    write(*,*) tb(i)/db(i) , tb(nlp)/db(nlp) , pij
!                    write(*,*) mass(i) , mass(nlp)
!                    write(*,*) db(i) , db(nlp)
!                    write(*,*) tb(i) , tb(nlp)
!                 endif
     ...
     if(i ==1 ) write(33,*) "next step"
     
     
     cs(i)=dsqrt(vn2*tb(i)*gamma)
...
real*8 function  dw(a)
  ...
  real(8):: a,pi
!  pi = 3.141592d0
!  dw=- 1d0 / dsqrt(2d0*pi) *dabs(b)*dexp(-b**2/2)
!  return
